...
- _**한가지 특이점은 10부터 최대 133까지 직업의 변화 없이 하나의 직업으로만 플레이한 유저도 상당 수 존재함**_
...
- _**주어진 데이터에서 캐릭터별 10개씩의 스냅샷 로그가 연속적인 것인지 중간에 누락된 것들이 있는 것인지 알 수 없음**_
...
- 데이터 종류
    - 해당 로그 데이터는 넥슨에서 서비스 중인 메이플스토리의 로그로 예상됨
